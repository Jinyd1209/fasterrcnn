import warnings
from collections import OrderedDict #实现对字典的排序
...
class FasterRCNNBase(nn.Module):
    ...
    def eager_outputs(self, losses, detections):
        ...
        if self.training: # 判断是否处于训练模式即model.train()
            return losses
    ...
    def forward(self, images, targets=None):
        ...
        if self.training:
            ...
            for target in targets:         # 进一步判断传入的target的boxes参数是否符合规定
                ...
                if isinstance(boxes, torch.Tensor):
                    if len(boxes.shape) != 2 or boxes.shape[-1] != 4: # boxes必须是二维tensor且列只能为四即四个坐标值
        ...
        for img in images:
            ...
            original_image_sizes.append((val[0], val[1])) # 每一张图片的尺寸存储为一个元组
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        if not hasattr(backbone, "out_channels"): # 判断backbone类内是否有out_channel属性
            raise ValueError(
        ...
        if num_classes is not None:
            if box_predictor is not None: # 当num_class不为空时需要自己初始化box_predictor
