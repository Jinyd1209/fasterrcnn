...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def forward(self,
        ...
        # 记录resize后的图像尺寸，方便在输出结果的时候变回原来的尺寸
        image_sizes = [img.shape[-2:] for img in images]
        #images的尺寸为[batchs,3,H,W],H,W为padding后统一的大小
        images = self.batch_images(images)  # 将images打包成一个batch
        image_sizes_list = torch.jit.annotate(List[Tuple[int, int]], []) # 定义一个变量
