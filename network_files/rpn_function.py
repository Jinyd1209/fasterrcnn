...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        #None扩充维度，w_rations[:,None].shape=[3,1],scales[None,:].shape=[1,3],相乘得到[3,3]的tensor
        # view(-1)将tensor变为一维
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            # 生成的shifts_anchor为[850,15,4],利用广播机制将每个点上的所有锚框都加上偏移量，850表示feature map大小，15表示每个点生成的锚框数
            # 4表示坐标
    ...
    def cached_grid_anchors(self, grid_sizes, strides):
        ...
        anchors = self.grid_anchors(grid_sizes, strides) #list[tensor(特征图上所有锚框数，4)]
...
class RPNHead(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = []
        bbox_reg = []
        for i, feature in enumerate(x): # 针对FPN网络生成的多个特征层进行遍历
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        # 得到取样后的正负样本的索引列表蒙版，列表的元素个数表示batch大小，在选取的正或负样本处置为1
    ...
    def forward(self,
        ...
        num_anchors_per_level_shape_tensors = [o[0].shape for o in objectness] #得到每个特征层的三维大小
