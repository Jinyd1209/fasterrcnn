...
class FeaturePyramidNetwork(nn.Module):
    ...
    def forward(self, x: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        for idx in range(len(x) - 2, -1, -1):
            #通过1x1的卷积层调整每一层feature map的通道数相同
            inner_lateral = self.get_result_from_inner_blocks(x[idx], idx)
            feat_shape = inner_lateral.shape[-2:]
            #通过最邻近插值法上采样该层下一层的feature map
            inner_top_down = F.interpolate(last_inner, size=feat_shape, mode="nearest")
            last_inner = inner_lateral + inner_top_down # feature map对应元素相加
            results.insert(0, self.get_result_from_layer_blocks(last_inner, idx)) # 相加后通过3x3的卷积得到预测特征层
